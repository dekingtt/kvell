...
#undef XX
    ...
    kvell::ConfigVar<std::set<LogDefine>>::ptr g_log_defines =
        kvell::Config::Lookup("logs", std::set<LogDefine>(), "logs config");
    ...
    struct LogIniter
    {
        LogIniter()
        {
            g_log_defines->addListener([](const std::set<LogDefine> &old_value,
                                       ...
                                       {
            KV_LOG_INFO(KV_LOG_ROOT()) << "on_logger_conf_changed";
            for(auto& i : new_value) {
                auto it = old_value.find(i);
                kvell::Logger::ptr logger;
                if(it == old_value.end()) {
                    //新增logger
                    logger = KV_LOG_NAME(i.name);
                } else {
                    if(!(i == *it)) {
                        //修改的logger
                        logger = KV_LOG_NAME(i.name);
                ...
                for(auto& a : i.appenders) {
                    kvell::LogAppender::ptr ap;
                    if(a.type == 1) {
                        ap.reset(new FileLogAppender(a.file));
                    } else if(a.type == 2) {
                        if(!kvell::EnvMgr::GetInstance()->has("d")) {
            ...
            for(auto& i : old_value) {
                ...
                if(it == new_value.end()) {
                    //删除logger
                    auto logger = KV_LOG_NAME(i.name);
